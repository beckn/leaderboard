...
class GitHubScraper:
    ...
    def parse_event(self, event, event_time):
        ...
        self.log.debug(f"Parsing event user: {user}")
        try:
    ...
    def fetch_events(self, page=1):
        ...
        events_count = 0
        self.log.info(f"Fetched preprocessed{len(events)} events")
        for event in events:
    ...
    def scrape(self):
        ...
        self.fetch_events(1)
        self.log.info(f"Scraping open pull requests for {self.data.keys()} users")
